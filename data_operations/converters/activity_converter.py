...
class ActivityConverter(BaseEntityConverter[ActivityIn]):
    ...
    JSON_KEY_CANDIDATES_FOR_NAME_FIELD = ["hasName", "name"]
    ...
    def convert(self, json_entity: Dict[str, Any]) -> ActivityIn:
        ...
        pure_name = self._get_optional_field_value(json_entity, self.JSON_KEY_CANDIDATES_FOR_NAME_FIELD)
        external_id = self._get_external_id(json_entity)
        if external_id:
            clean_name = pure_name if pure_name else remove_prefix(external_id)
        ...
        processed_clean_keys.extend(self.JSON_KEY_CANDIDATES_FOR_NAME_FIELD)
        ...
        if DEBUG:
            print(
                f"📝 Creating ActivityIn: activity='{activity_type}', external_id='{activity.external_id}', import_job_id='{activity.import_job_id}', properties={len(additional_properties)} (including common)")
        return activity
